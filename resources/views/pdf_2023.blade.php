        ...
        <form class="row g-1 " action="#" style="padding: 3px;">
            <div class="d-flex flex-row-reverse mb-1" >
                <div class="mt-2 border border-dark" style="height: 26px; width: 300px">
                    <p class="">FOLIO: @if ($pago->payment_folio <> 'null' || $pago->payment_folio == null) {{ $pago->payment_folio }} @endif</p>
                </div>
            </div>
            ...
            {{-- {{ date_format(date_create($order->order_date), 'd-m-Y') }} --}}
            <div class="col-6  rounded text-center" style="height: 26px">
                <label for="createdDate" class="form-label">FECHA DE EXPEDICIÓN: {{ date_format(date_create($pago->updated_at), 'd-m-Y') }}</label>
            ...
            <div class="col-12  rounded pb-" style="height: 38px">
                <p style="font-size: 10px; margin-bottom: 10px;" class="text-uppercase"> EL CIUDADANO: <b style="font-size: 14px"> <u> {{ $pago->payment_name }} </u> </b>
                    <br>
                    EFECTUA EL PAGO EN LA TESORERÍA MUNICIPAL POR LA CANTIDAD DE: <b style="font-size: 14px"> &nbsp; $<u>{{ number_format($pago->payment_amount, 2 ) }} MXN</u></b> 
                    &nbsp; &nbsp; POR EL CONCEPTO DE:
                </p>
            ...
            <div class="col-md-12">
                <div class="col-12  rounded d-flex justify-content-start row mx-1 border border-3 border-dark" style="height: auto">
                    ...
                    <p class="d-flex align-items-center col-12 text-uppercase" style="font-size: 14px;" >
                        DÍAS LABORADOS = {{ $pago->payment_daysWorked }} * COSTO FISCAL =  ${{ $pago->payment_fiscalAmount }} * {{ $pago->payment_dimentions }} 
                        @if ($pago->payment_fiscalAmountExtraMeters <> 0) (${{ $pago->payment_fiscalAmountExtraMeters }})@endif, 
                        AJUSTE TARIFARIO = ${{ $pago->payment_tarifAdjustment }}, 
                        TOTAL = ${{ number_format($pago->payment_amount, 2) }}
                    </p>
                </div>
            </div>
