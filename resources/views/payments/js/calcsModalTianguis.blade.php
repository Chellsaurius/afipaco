<script>
    function newValueTia(id) {
        ...
        // console.log(document.getElementById("IDatePayment" + id));
        var fecha = document.getElementById("FDatePayment" + id).value;
        if (fecha != '') {
            // console.log('entró, cambio en los días');
            daysDifferenceTia(id)
        }
    }
</script>
<script>
    function activateDateEndTia(id) {
        // console.log(id)
        if (this.value == '0') {
...
<script>
    function daysDifferenceTia(id) {
        ...
        const start = date1;
        const end = date2;
        // console.log('las fechas del loop son: ' + start + ', ' + end)
